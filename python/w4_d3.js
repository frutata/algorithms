...
function binarySearch(sortedNums, searchNum) {
    ...
    // if value at middle equals search num, search is complete
    if(sortedNums[mid] == searchNum){
        return true;
    }
    // if search num > value at mid, move search range to right half of array
    else if(searchNum > sortedNums[mid]){
        return binarySearch(sortedNums.slice(mid, sortedNums.length-1), searchNum);
    }
    // if search num < value at mid, move search to left half of array
    else if(searchNum < sortedNums[mid]){
